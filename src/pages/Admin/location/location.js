    ...
    return (
        <div className={cx('client')}>
            <div className={cx('tagBar')}>
                <button onClick={handleStyleFontAddLocation}><FontAwesomeIcon icon={faPlus} /> Thêm mới</button>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        {userDataLocation.length === 0 ? (
                            ...
                            userDataLocation.map((product, indext) => {
                                    ...
                                    <tr key={indext}>
                                       ...
                                       <td>{`${product.city}-${product.district}-${product.ward}-${product.street}`}</td>
                                        <td className={cx("detail")}  >
                                            <button className={cx("buttonview")} onClick={()=>handleLocationId(product._id)} >
                                                <FontAwesomeIcon icon={faEllipsis} />
                                            </button>
                                            <div className={cx(idDetailLocation!==product._id?"noneTableDetail":"tableDetail")}>
                                               <div className={cx("buttonX")}onClick={()=>handleLocationId("default")} >
                                                 <button >
                                                <FontAwesomeIcon icon={faX} />
                                                </button>
                                               </div>
                                               <button  className={cx("buttonDetail")} onClick={()=>callAPILocationDetail(product._id)}>
                                               <FontAwesomeIcon icon={faUserCheck} />
                                               <h5>Chi tiết</h5>
                                               </button>
                                                <button className={cx("buttonFix")} onClick={()=>handleStyleFontPutLocation(product._id)}>
