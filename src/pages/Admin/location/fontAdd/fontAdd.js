...
const FormAddLocation = () => {
  const {handleStyleFontAddLocation,setUserLocation,modalSaveLocation,handleSaveLocation}=useContext(AppContext)
const week = [
    {
        id: 1,
        day: 'Thứ 2',
  ...
  const [ times_days,]=useState([]);
    const formick = useFormik({
       ...
       try {
        ...
        await postUser.postDataLocation(value.name,value.city,value.district,value.ward,value.street,
          value.phone,value.img,value.desc, times_days,accessToken,refresh_token
          )
