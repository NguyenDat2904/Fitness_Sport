...
import {faUserXmark,faToolbox,faUserCheck,faX, faKeyboard, faCaretDown, faEllipsis } from '@fortawesome/free-solid-svg-icons';
...
const Client = () => {
   const {userData,
    ...
    tableDetailId,
    handleTableStyle,
    setTacbleUser}=useContext(AppContext)
    ...
    const result = userData.filter((product) =>{
        if(selectCourse&&selectLever){
             return product.courseID.filter((item)=>item.name===selectCourse).length>0 && product.rank === selectLever
        }
        if(!selectCourse&&selectLever){
        ...
        <div className={cx('client')}>
            <div className={cx('tagBar')}>
                ...
                <select className={cx('selectSort')} name="" id="" onChange={onChangeSelectSort}>
                    <option className={cx('opacity')}   value="">Sắp xếp</option>
              ...
              {
                putNewStyle===false? <FormPut setUser={setUser}  locationData={locationData} benefitData={benefitData} idPut={idPut} courseData={courseData}  handlePutNew={handlePutNew}/>:
                            ...
                            result.map((product, indext) => {
                                ...
                                return (
                                    <tr key={indext}>
                                        ...
                                        {product.courseID.length===0?<td>null</td>:<td>{`${product.courseID[0].name}...`}</td> }
                                       <td>{product.rank===""?"null":product.rank}</td>
                                        <td>{product.updatedAt.slice(0, 10)}</td>
                                        <td className={cx("detail")}  >
                                            <button className={cx("buttonview")} onClick={()=>handleTableStyle(product._id)}>
                                                <FontAwesomeIcon icon={faEllipsis} />
                                            </button>
                                            <div className={cx(tableDetailId!==product._id?"noneTableDetail":"tableDetail")}>
                                               <div className={cx("buttonX")}onClick={()=>handleTableStyle("default")}>
                                                 <button >
                                                <FontAwesomeIcon icon={faX} />
                                                </button>
                                               </div>
                                               
                                                <button className={cx("buttonDetail")} onClick={()=>handleTurnOnDetail(product._id)} >
                                                <FontAwesomeIcon icon={faUserCheck} />
                                                <h5>Chi tiết</h5>
                                                </button>
                                                <button className={cx("buttonFix")} onClick={()=>handlePutNew(product)}>
