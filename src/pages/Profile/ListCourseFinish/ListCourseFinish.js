...
import style from './ListCourseFinish.module.scss';
...
function ListCourseFinish() {
    const { loadingProfile, userInfoProfile } = useContext(AppContext);
    const courseActive = userInfoProfile.courseID?.filter((course) => course.status === 'Hoàn thành');
    const renderCourse =
                    ...
                    <>
                        {renderCourse !== null ? (
                            ...
                            <div className={cx('course-link')}>
                                Bạn chưa hoàn thành khoá học nào. <Link to="/course">Đăng ký ngay</Link>
                            </div>
                        )}
                    </>
                ) : (
...
export default ListCourseFinish;
