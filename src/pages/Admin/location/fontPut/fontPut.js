...
const FormPutLocation=()=>{
    const {handleStyleFontPutLocation,setUserLocation,modalSaveLocation,idputLocation,handleSaveLocation}=useContext(AppContext)
    const week = [
        {
            id: 1,
            day: 'Thứ 2',
      ...
      const [ times_days,]=useState([]);
        const formick = useFormik({
           ...
           try {
            ...
            await putdata.putDataLocation(value.name,value.city,value.district,value.ward,value.street,
              value.img,value.desc,value.phone, times_days,idputLocation,accessToken,refresh_token
              )
        ...
        const handleSubmitSchedule=()=>{
          ...
          if(formick.values.start_time!==""&&formick.values.end_time!==""&&
          formick.values.start_day!=="" && formick.values.end_day!==""
          ){
                ...
                <div className={cx('formAddInput')}>
                    <div className={cx('formAddInPutLeft')}>
                        <div className={cx('input')}>
                            ...
                            <input name="name" type="text" placeholder="Tên chi nhánh mới..." onChange={formick.handleChange} />
                        </div>
                        <div className={cx('input')}>
                            {formick.errors.city && formick.touched.city && (<div className={cx("error")}><p className={cx("pError")}>{formick.errors.city}</p></div>)}
                            <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                            <input
                                name="city"
                                type="text"
                        ...
                        <div className={cx('input')}>
                            {formick.errors.district && formick.touched.district && (<div className={cx("error")}><p className={cx("pError")}>{formick.errors.district}</p></div> )}
                            <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                            
                            <input name="district" type="text" placeholder='Tại Quận/Huyện...' onChange={formick.handleChange} />
                        </div>
                        <div className={cx('input')}>
                            {formick.errors.ward && formick.touched.ward && (<div className={cx("error")}><p className={cx("pError")}>{formick.errors.ward}</p></div> )}
                            <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                           
                            <input name="ward" type="text" placeholder='Tại phường...' onChange={formick.handleChange} />
                        </div>
                        <div className={cx('input')}>
                            {formick.errors.street && formick.touched.street && (<div className={cx("error")}><p className={cx("pError")}>{formick.errors.street}</p></div>)}
                            <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                        ...
                        <div className={cx('input')}>
                            {formick.errors.phone && formick.touched.phone && (<div className={cx("error")}><p className={cx("pError")}>{formick.errors.phone}</p></div> )}
                            <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                            <input name="phone" id="" placeholder='Số điện thoại của chi nhánh...' onChange={formick.handleChange}/>
                    ...
                    <div className={cx('formAddInPutRight')}>
                        ...
                        <div className={cx('inputSpan')}>
                            {formick.errors.end_time && formick.touched.end_day && (
                                 <div className={cx("error")}>  <p className={cx("pError")}>{formick.errors.end_time}</p></div>
                        ...
                        <div className={cx("input")}>
                            {formick.errors.start_day && formick.touched.start_day && (
                                 <div className={cx("error")}> <p className={cx("pError")}>{formick.errors.start_day}</p></div>
                            
                            )}
                             <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                            <select  name="start_day" id=""  onChange={formick.handleChange}>
                                <option className={cx('opacity')} value="">Bắt đầu từ thứ...</option>
                                {week.map((product)=>{
                                    return <option key={product.id} value={product.day}>{product.day}</option>
                                })}
                            </select>
                        ...
                        <div className={cx("input")}>
                            {formick.errors.end_day && formick.touched.end_day && (
                                 <div className={cx("error")}><p className={cx("pError")}>{formick.errors.end_day}</p></div>
                                
                            )}
                             <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                            <select name="end_day" id=""  onChange={formick.handleChange}>
                                <option className={cx('opacity')} value="">Kết thúc từ thứ...</option>
                                {week.map((product)=>{
                                    return <option key={product.id} value={product.day}>{product.day}</option>
                                })}
                            </select>
                        ...
                        <div className={cx("buttonSchedule")}>
                          <p>{notification}</p>
                           <button type='button' onClick={handleSubmitSchedule} > Lưu thời gian hoạt động</button>
                        </div>
                        <div  className={cx("input")}>
                        ...
                        <div className={cx("input")}>
                            {formick.errors.desc && formick.touched.desc && (
                                 <div className={cx("error")}> <p className={cx("pError")}>{formick.errors.desc}</p></div>
                               
                            )}
                              <FontAwesomeIcon icon={faStarOfLife} className={cx('icon')} />
                            <textarea className={cx('textares')} name="desc" id="" cols="30" rows="5" placeholder='Mô tả...' onChange={formick.handleChange}/>
                        </div>
                    </div>
                </div>
